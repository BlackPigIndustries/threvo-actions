...
BEGIN
    ...
    WHILE valid AND sequence_index < JSON_LENGTH(JSON_EXTRACT(
        ...
        IF NOT COALESCE((
            ...
            (
                JSON_LENGTH(current_value) = 14 OR (
                    JSON_LENGTH(current_value) = 15 AND
                    JSON_CONTAINS_PATH(current_value, 'one', '$.external_attestation') = 1
                )
            ) AND
            JSON_CONTAINS_PATH(
            ...
            JSON_UNQUOTE(JSON_EXTRACT(current_value, '$.proposal_commitment')) <=>
                JSON_UNQUOTE(JSON_EXTRACT(p_proposal_data, '$.commitment.digest')) AND
            (
                JSON_CONTAINS_PATH(current_value, 'one', '$.external_attestation') = 0 OR
                JSON_TYPE(JSON_EXTRACT(current_value, '$.external_attestation')) = 'NULL' OR (
                    JSON_TYPE(JSON_EXTRACT(current_value, '$.external_attestation')) = 'OBJECT' AND
                    JSON_LENGTH(JSON_EXTRACT(current_value, '$.external_attestation')) = 4 AND
                    JSON_CONTAINS_PATH(
                        JSON_EXTRACT(current_value, '$.external_attestation'), 'all',
                        '$.format', '$.issuer_reference', '$.artifact_reference', '$.artifact_digest'
                    ) = 1
                )
            )
        ), FALSE) THEN SET valid = FALSE; END IF;
        ...
        IF JSON_TYPE(JSON_EXTRACT(current_value, '$.external_attestation')) = 'OBJECT' THEN
            SET safe_values = JSON_ARRAY_APPEND(
                safe_values,
                '$', JSON_EXTRACT(current_value, '$.external_attestation.format'),
                '$', JSON_EXTRACT(current_value, '$.external_attestation.issuer_reference'),
                '$', JSON_EXTRACT(current_value, '$.external_attestation.artifact_reference'),
                '$', JSON_EXTRACT(current_value, '$.external_attestation.artifact_digest')
            );
        END IF;
        SET nested_index = 0;
