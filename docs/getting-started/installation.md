...
!!! note "Release availability"
    ...
    These commands install the immutable `0.6.1` release after the signed tag
...
=== "Stripe refunds"
    ...
    and [runnable sandbox app](../examples/stripe-refunds.md).
...
=== "AWS KMS protection"
    ...
    ```bash
    uv add "threvo-actions[aws-kms]==0.6.1"
    ```
...
=== "Local KEK protection"

    ```bash
    uv add "threvo-actions[local-kek]==0.6.1"
    ```

    Use this when the host already keeps a versioned 256-bit KEK in its secret
    infrastructure. The library owns envelope encryption and the host owns KEK
    retrieval plus durable wrapped-key storage. See the
    [local KEK guide](../integrations/local-kek.md).

=== "PostgreSQL"

    ```bash
    uv add "threvo-actions[postgres]==0.6.1"
    ```

=== "MySQL"

    ```bash
    uv add "threvo-actions[mysql]==0.6.1"
    ```
...
=== "SQLAlchemy and Alembic"
    ...
    ```bash
    uv add "threvo-actions[sqlalchemy]==0.6.1"
    ```
...
=== "Pydantic AI"
    ...
    ```bash
    uv add "threvo-actions[pydantic-ai]==0.6.1"
    ```
