...
| Store security profile | Store adapter + deployment | Official profiles expose tested topology, privilege separation, and explicit data-handling exclusions. They do not configure encryption, authenticate evidence issuers, erase external copies, or certify the deployment. |
