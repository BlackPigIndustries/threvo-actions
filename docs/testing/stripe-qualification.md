...
| Schedule cancellation | Exact active or trialing subscription is updated for the observed period end | Bound subscription state and correlation both match | Desired state with missing/different correlation is `PROVISIONAL_ABSENCE` | No later termination is inferred; host investigation remains required |
